...
ids.each do |id|
   ...
   blob = read_blob( "./minis/inscribe/mini#{id}.png" )
   ...
   inscriptionNumber =  if File.exist?( path )
                            count += 1
                            meta = read_json("./minis/hashcheck/#{id}.json" )
   ...
   items << { id: hash,
              name: "##{id}",
              image: "https://github.com/orc721/monkes21/raw/master/minis/inscribe/mini#{id}.png",
