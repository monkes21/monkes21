...
ids.each do |id|
   ...
   blob = read_blob( "./pennies/inscribe/penny#{id}.png" )
   ...
   inscriptionNumber =  if File.exist?( path )
                            count += 1
                            meta = read_json("./pennies/hashcheck/#{id}.json" )
   ...
   items << { id: hash,
              name: "##{id}",
              image: "https://github.com/orc721/monkes21/raw/master/pennies/inscribe/penny#{id}.png",
